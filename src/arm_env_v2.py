"""Balanced two-joint arm renderer used after the documented v1 failure.

V1 allowed a predictor to model the larger proximal link while largely
ignoring the distal joint.  V2 equalizes link extent and thickness; training
also gives distal-link pixels explicit coverage.  Dynamics are unchanged.
"""
...
def render(q, palette=0, lighting=1.0, noise=0.01, rng=None):
    ...
    image = np.where((d1 < 0.078)[..., None], c1, image)
    image = np.where((d2 < 0.092)[..., None], c2, image)
    for cx, cy, color, radius in (
            (x0, y0, np.asarray((0.90, 0.90, 0.90), np.float32), 0.10),
            (x1, y1, np.asarray((0.98, 0.78, 0.18), np.float32), 0.095),
            (x2, y2, np.asarray((0.30, 0.95, 0.42), np.float32), 0.090)):
